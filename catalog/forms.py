...
class RenewBookForm(forms.Form):
    ...
    def clean_renewal_date(self):
        ...
        if data > datetime.date.today() + datetime.timedelta(weeks = 4):
            raise ValidationError(_('Invalid date - renewal more than designated period'))
...
class RenewBookModelForm(ModelForm):
    ...
    def clean_due_back(self):
        ...
        # Check if a date is not in the past
        if data < datetime.date.today():
            raise ValidationError(_('Invalide date - renewal in the past'))

        # Check if a date is in the allowed range
        if date > datetime.date.today() + datetime.timedelta(weeks=4):
            raise ValidationError(_('Invalide date - renewal date more than designated limit'))
